...
│  │ └── Trust level: ZERO (UX optimization only)            │    │
│  └─────────────────────────────────────────────────────────┘    │
│                         ↕                                       │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │ Client SDK                                              │    │
...
- [x] Add server-side KuzuDB to Durable Object
- [x] Implement validatePermission() method
- [x] Reload server graph after mutations
- [x] Add validateCRUDOperation() helper
- [x] Add /validate endpoint

Security:

- [ ] Add authentication middleware (extract userId from HTTP-only cookies)
- [ ] Add JWT validation to all endpoints
- [ ] Implement rate limiting (per-client and per-org)
...
- [ ] Server-side validation stress tests
- [ ] DO eviction and recovery testing

Monitoring:

- [ ] Metrics collection (validation latency, success rate)
...
- [x] Security model documentation (SECURITY.md)
- [x] Graph architecture documentation (GRAPH_ARCHITECTURE.md)
- [x] Server validation guide (SERVER_VALIDATION.md)
- [ ] Operations runbook
- [ ] Deployment guide
