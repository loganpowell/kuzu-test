...
async function handleRegister(e: Event): Promise<void> {
  ...
  const displayName = (form.querySelector('#register-name') as HTMLInputElement).value;
...
function updateDashboard(): void {
  ...
  if (userStatusEl) {
    userStatusEl.textContent = currentUser.emailVerified ? 'Verified' : 'Not Verified';
    userStatusEl.className = currentUser.emailVerified ? 'status-verified' : 'status-unverified';
  }
}
