...
class GraphStateCSV {
  ...
  async handleCRUDOperation(
    ...
    const allowed = await this.validatePermission(userId, operation, resourceId);
    ...
    if (mutation.type === 'grant') {
      await this.state.storage.sql.exec(
        `INSERT OR REPLACE INTO permissions (user_id, resource_id, permission, granted_at)
         VALUES (?, ?, ?, ?)`,
        mutation.user, mutation.resource, mutation.permission, Date.now()
      );
    } else if (mutation.type === 'revoke') {
      await this.state.storage.sql.exec(
        `DELETE FROM permissions 
         WHERE user_id = ? AND resource_id = ? AND permission = ?`,
        mutation.user, mutation.resource, mutation.permission
      );
    }
