...
### Alternative Considered: "Just use server validation"

**Could we skip client-side graph entirely and always query server?**

```
Option A: Server-only (No client KuzuDB)
  User: "Can I see edit button?"
    → Network request (50ms)
    → Server validates
    → Response
  Result: 50ms delay for every UI decision
  
Option B: Client + Server (Our approach)
  User: "Can I see edit button?"
    → Client KuzuDB (0ms)
  ...
  User: "Actually edit the document"
    → Network request (50ms)
    → Server validates with server KuzuDB
    → Execute or reject
  Result: 0ms UI, 50ms validation (only when needed)
```

**Tradeoff**: Small memory overhead (10-50MB per org) for massive UX improvement.

### Summary

- ✅ HTTP-only cookies: Essential for protecting session tokens
- ✅ Authenticated CSV downloads: Essential for org data isolation
- ✅ Client-side graph: Essential for fast UX
- ✅ Server-side graph: Essential for security
- ❌ Skipping server validation: Not possible, regardless of authentication method

The client is always untrusted. Authentication determines **which data** the user can access, not **whether we trust their copy** of that data.
